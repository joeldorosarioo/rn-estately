	...
	return (
		<TouchableOpacity onPress={onPress} className="flex flex-col items-start w-60 h-80 relative">
			...
			<View className="flex flex-col items-start absolute bottom-5 inset-x-5">
				<Text className="text-xl font-rubik-extrabold text-white" numberOfLines={1}>{item.title}</Text>
				<Text className="text-base font-rubik text-white" numberOfLines={1}>{item.location}</Text>
	...
	return (
		<TouchableOpacity className="flex-1 w-full mt-4 px-3 py-4 rounded-lg bg-white shadow shadow-black-100/70 relative" onPress={onPress}>
			...
			<View className="flex flex-col mt-2">
				<Text className="text-base font-rubik-bold text-black-300">{item.title}</Text>
				<Text className="text-xs font-rubik text-black-100">{item.location}</Text>
